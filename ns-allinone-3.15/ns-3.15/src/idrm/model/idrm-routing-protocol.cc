...
NS_LOG_COMPONENT_DEFINE ("IdrmRoutingProtocol");

namespace ns3 {
namespace idrm {
NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for IDRM control traffic
const uint32_t RoutingProtocol::IDRM_PORT = 269;

/// Tag used by IDRM implementation
struct DeferredRouteOutputTag : public Tag
{
  ...
  GetTypeId ()
  {
    static TypeId tid = TypeId ("ns3::idrm::DeferredRouteOutputTag").SetParent<Tag> ();
    return tid;
  }
...
{
  static TypeId tid = TypeId ("ns3::idrm::RoutingProtocol")
  ...
  if (m_socketAddresses.empty ())
    {
      ...
      NS_LOG_LOGIC ("No idrm interfaces");
      Ptr<Ipv4Route> route;
      return route;
    }
                             ...
                             ErrorCallback ecb)
{
  ...
  if (m_socketAddresses.empty ())
    {
      NS_LOG_DEBUG ("No idrm interfaces");
      return false;
    }
  NS_ASSERT (m_ipv4 != 0);
  ...
  // IDRM is not a multicast routing protocol
  if (dst.IsMulticast ())
    {
      return false;
    }
  ...
  // LOCAL DELIVARY TO IDRM INTERFACES
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif) const
{
  ...
  // returned when IDRM does not have a route; this causes the packet
  // to be looped back and handled (cached) in RouteInput() method
  // while a route is found. However, connection-oriented protocols
  // like TCP need to create an endpoint four-tuple (src, src port,
  // dst, dst port) and create a pseudo-header for checksumming.  So,
  // IDRM needs to guess correctly what the eventual source address
  // will be.
  //
  // For single interface, single address nodes, this is not a problem.
  // When there are possibly multiple outgoing interfaces, the policy
  // implemented here is to pick the first available IDRM interface.
  ...
  NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid IDRM source address not found");
...
void
RoutingProtocol::RecvIdrm (Ptr<Socket> socket)
{
  ...
  IdrmHeader routeCount;
  	//cout << "packet size: " << packetSize << endl;
  packet->RemoveHeader (routeCount);
  //cout << "route count: " << routeCount.GetDstSeqno() << endl;
  packetSize -= 12;
  //std::cout<< routeCount.GetDstSeqno() <<endl; 
  for (int i = 0; i < (int)routeCount.GetDstSeqno(); i++)
    {   
      //cout << "current i: "<<i <<endl;
      count = 0;
      IdrmHeader idrmHeader, tempIdrmHeader;
	  //cout << packet->GetSize()<<endl;
      packet->RemoveHeader (idrmHeader);
      packetSize = packetSize - 12;
      NS_LOG_DEBUG ("Processing new update for " << idrmHeader.GetDst ());
      ...
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j
           != m_socketAddresses.end (); ++j)
        {
          ...
          if (idrmHeader.GetDst () == interface.GetLocal ())
            {
              if (idrmHeader.GetDstSeqno () % 2 == 1)
                {
                  NS_LOG_DEBUG ("Sent Idrm update back to the same Destination, "
      ...
      NS_LOG_DEBUG ("Received a IDRM packet from "
                    << sender << " to " << receiver << ". Details are: Destination: " << idrmHeader.GetDst () << ", Seq No: "
                    << idrmHeader.GetDstSeqno () << ", HopCount: " << idrmHeader.GetHopCount ());
      RoutingTableEntry fwdTableEntry, advTableEntry;
      EventId event;
      bool permanentTableVerifier = m_routingTable.LookupRoute (idrmHeader.GetDst (),fwdTableEntry);
      if (permanentTableVerifier == false)
          ...
          if (idrmHeader.GetDstSeqno () % 2 != 1)
            {
              ...
              RoutingTableEntry newEntry (
                /*device=*/ dev, /*dst=*/
                idrmHeader.GetDst (), /*seqno=*/
                idrmHeader.GetDstSeqno (),
                /*iface=*/ m_ipv4->GetAddress (m_ipv4->GetInterfaceForAddress (receiver), 0),
                /*hops=*/ idrmHeader.GetHopCount (), /*next hop=*/
                sender, /*lifetime=*/
        ...
        {
          if (!m_advRoutingTable.LookupRoute (idrmHeader.GetDst (),advTableEntry))
            {
              ...
              m_advRoutingTable.LookupRoute (idrmHeader.GetDst (),advTableEntry);
            }
          if (idrmHeader.GetDstSeqno () % 2 != 1)
            {
              if (idrmHeader.GetDstSeqno () > advTableEntry.GetSeqNo ())
                {
                  // Received update with better seq number. Clear any old events that are running
                  if (m_advRoutingTable.ForceDeleteIpv4Event (idrmHeader.GetDst ()))
                    {
                  ...
                  if (idrmHeader.GetHopCount () != advTableEntry.GetHop ())
                    {
                      advTableEntry.SetSeqNo (idrmHeader.GetDstSeqno ());
                      ...
                      advTableEntry.SetHop (idrmHeader.GetHopCount ());
                      NS_LOG_DEBUG ("Received update with better sequence number and changed metric.Waiting for WST");
                      Time tempSettlingtime = GetSettlingTime (idrmHeader.GetDst ());
                      ...
                      m_advRoutingTable.AddIpv4Event (idrmHeader.GetDst (),event);
              ...
              else if (idrmHeader.GetDstSeqno () == advTableEntry.GetSeqNo ())
                {
                  if (idrmHeader.GetHopCount () < advTableEntry.GetHop ())
                    {
                      ...
                      m_advRoutingTable.ForceDeleteIpv4Event (idrmHeader.GetDst ());
                      advTableEntry.SetSeqNo (idrmHeader.GetDstSeqno ());
                      advTableEntry.SetLifeTime (Simulator::Now ());
                      advTableEntry.SetFlag (VALID);
                      advTableEntry.SetEntriesChanged (true);
                  ...
                  else
                    {
                       ...
                       * Discard that update.
                       */
                      if (not m_advRoutingTable.AnyRunningEvent (idrmHeader.GetDst ()))
                        {
                      ...
                      NS_LOG_DEBUG ("Received update with same seq number and "
                                    "same/worst metric for, " << idrmHeader.GetDst () << ". Discarding the update.");
                    }
                }
              else
                {
                  ...
                  if (not m_advRoutingTable.AnyRunningEvent (idrmHeader.GetDst ()))
                    {
                      m_advRoutingTable.DeleteRoute (idrmHeader.GetDst ());
                    }
                  NS_LOG_DEBUG (idrmHeader.GetDst () << " : Received update with old seq number. Discarding the update.");
                }
            }
          else
            {
              NS_LOG_DEBUG ("Route with infinite metric received for "
                            << idrmHeader.GetDst () << " from " << sender);
              ...
              if (sender == advTableEntry.GetNextHop ())
                {
                  ...
                  m_routingTable.GetListOfDestinationWithNextHop (idrmHeader.GetDst (),dstsWithNextHopSrc);
                  m_routingTable.DeleteRoute (idrmHeader.GetDst ());
                  advTableEntry.SetSeqNo (idrmHeader.GetDstSeqno ());
                  advTableEntry.SetEntriesChanged (true);
                  m_advRoutingTable.Update (advTableEntry);
              ...
              else
                {
                  ...
                  NS_LOG_DEBUG (idrmHeader.GetDst () <<
  ...
  for(std::map<Ipv4Address, int>::const_iterator i = m_nc.begin (); i != m_nc.end (); ++i)
  	{
  	//std::cout << "Address: " << i->first << " Neighbor:  " << i->second <<endl;
  	 if(i->second > m_nc[receiver] && i->first != receiver)
  	 	{
  ...
  for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j
       != m_socketAddresses.end (); ++j)
    {
	  ...
	  for(std::map<Ipv4Address, int>::const_iterator i = m_nc.begin (); i != m_nc.end (); ++i)
	  {
		...
		idrmHeader.SetDstSeqno((uint32_t)i->second);
      ...
      if (packet->GetSize () >= 12)
        {
          ...
          packet->AddHeader (idrmHeader);
		  //routeCount++;
		  idrmHeader.SetDstSeqno(uint32_t(routeCount));
		  packet->AddHeader (idrmHeader);
          ...
          socket->SendTo (packet, 0, InetSocketAddress (destination, IDRM_PORT));
      ...
      for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator i = allRoutes.begin (); i != allRoutes.end (); ++i)
        {
          ...
          NS_LOG_DEBUG ("Forwarding details are, Destination: " << idrmHeader.GetDst ()
                                                                << ", SeqNo:" << idrmHeader.GetDstSeqno ()
                                                                << ", HopCount:" << idrmHeader.GetHopCount ()
...
RoutingProtocol::NotifyInterfaceUp (uint32_t i)
{
  ...
  NS_ASSERT (socket != 0);
  socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvIdrm,this));
  socket->BindToNetDevice (l3->GetNetDevice (i));
  socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), IDRM_PORT));
	...
	RoutingProtocol::SetRoutingTable(iMRoutingTable iMtable)
{
    ...
    for (std::map<Ipv4Address, iMRoutingTableEntry>::const_iterator iter = iMtable.begin (); iter != iMtable.end (); iter++)
    {
      ...
      {
          //std::cout<<"enter if!!!!!!!!!!!!!!!!!\n";
          RoutingTableEntry rt (iter->second.dev, iter->second.destAddr, iter->second.m_seqNo, iter->second.interface, iter->second.distance, iter->second.nextAddr, iter->second.lifetime);
