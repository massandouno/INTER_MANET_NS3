...
NS_LOG_COMPONENT_DEFINE ("IdrmManetExample");

class IdrmManetExample
{
public:
  IdrmManetExample ();
...
int main (int argc, char **argv)
{
  ...
  std::string CSVfileName = "IdrmManetExample.csv";
  ...
  cmd.AddValue ("CSVfileName", "The name of the CSV output file name[Default:IdrmManetExample.csv]", CSVfileName);
  cmd.Parse (argc, argv);
...
IdrmManetExample::IdrmManetExample ()
  : bytesTotal (0),
    packetsReceived (0)
{
}
...
void
IdrmManetExample::ReceivePacket (Ptr <Socket> socket)
{
...
void
IdrmManetExample::CheckThroughput ()
{
  ...
  out.close ();
  packetsReceived = 0;
  Simulator::Schedule (Seconds (1.0), &IdrmManetExample::CheckThroughput, this);
}

Ptr <Socket>
IdrmManetExample::SetupPacketReceive (Ipv4Address addr, Ptr <Node> node)
{
  ...
  sink->Bind (local);
  sink->SetRecvCallback (MakeCallback ( &IdrmManetExample::ReceivePacket, this));

  return sink;
}

void
IdrmManetExample::CaseRun (uint32_t nWifis, uint32_t nSinks, double totalTime, std::string rate,
                           ...
                           double dataStart, bool printRoutes, std::string CSVfileName)
{
  ...
  std::string tr_name = "Idrm_Manet_" + t_nodes + "Nodes_" + sTotalTime + "SimTime";
...
void
IdrmManetExample::SetupMobility ()
{
...
void
IdrmManetExample::CreateDevices (std::string tr_name)
{
...
void
IdrmManetExample::InstallInternetStack (std::string tr_name)
{
  IdrmHelper idrm;
  idrm.Set ("PeriodicUpdateInterval", TimeValue (Seconds (m_periodicUpdateInterval)));
  idrm.Set ("SettlingTime", TimeValue (Seconds (m_settlingTime)));
  InternetStackHelper stack;
  stack.SetRoutingHelper (idrm); // has effect on the next Install ()
  stack.Install (nodes);
  ...
  if (m_printRoutes)
    {
      ...
      idrm.PrintRoutingTableAllAt (Seconds (m_periodicUpdateInterval), routingStream);
    }
}

void
IdrmManetExample::InstallApplications ()
{
